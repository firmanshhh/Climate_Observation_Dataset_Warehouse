...
def get_monthlydiff_dataset(path):
    ...
    df_anomali = df_homo[(df_homo['parameter'] == 'TEMP_24H_TN_C') &(df_homo['source'] == 'qc')].copy()
...
# Simpan hasil ke CSV
output_path = os.path.join(LONG_DIR, '07.TMIN_MONTHLY_DIFF_DB.csv')
monthlydiff_sel.to_csv(output_path, index=False)
summary_output_path = os.path.join(LONG_DIR, '07.TMIN_MONTHLY_DIFF_SUMMARY_DB.csv')
