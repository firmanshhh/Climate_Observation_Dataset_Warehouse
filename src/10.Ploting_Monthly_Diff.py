...
def plot_anomali_suhu(point_df, year, month, outdir):
    ...
    point_df_now  = point_df_now.dropna(subset=['diff'])
    total_stasiun = point_df_now['diff'].reset_index(drop=True)
    ...
    bulan_before = bulan_full_name[month - 1].upper() if month > 1 else bulan_full_name[12].upper()
    year_before  = year if month > 1 else year - 1
    ...
    ax_footer_center.text(0.5, 0.5, f'SELISIH SUHU UDARA RATA-RATA\nBULAN {bulan_now} {year_now}\nTERHADAP\n{bulan_before} {year_before}', transform=ax_footer_center.transAxes,
    ...
    outfile = os.path.join(outdir, f'MONTHLY_DIFF_TEMPERATURE_BULANAN_{year_now}_{bulan_now}.png')
...
PLOT_DIR        = os.path.join(LONG_DIR, 'PLOT_ANOMALI')
point_df       = pd.read_csv(os.path.join(LONG_DIR, "07.TEMPERATURE_MONTHLY_DIFF_DB.csv"))
import datetime
now     = datetime.datetime.now()
yearnow = now.year
for year in range(2023, yearnow + 1):
    for month in range(1, 13):
        DIROOUT = os.path.join(PLOT_DIR, str(year), f'{year}_{month:02d}')
        try:
